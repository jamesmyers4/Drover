...
## 2026-07-23 (S3) — No reusable multi-page route map, only single-seed-URL resolution

CONTEXT.md says returning/veteran personas get "seeded with treeLine's known route map" (a crawl of the app). The adapter's real exposed surface is `resolveSeedUrl` (one URL in, resolved URL + that page's HTML out) — there's no crawl/site-map export. `src/actor/route-map.ts`'s `buildRouteMapContext` approximates a route map by regex-extracting `<a href>` targets from the single seed page's HTML, capped at 20. This is a much thinner "map" than treeLine actually builds internally during a real crawl. Ask: export the crawl's discovered-route list (paths only, no full capture data) from `@treeline/acquire` so returning/veteran personas can be seeded with what treeLine really knows, not a same-page link scrape.
