...
**Status: done, 2026-07-29 — not yet committed, review the diff.** The Clerk-auth question was real (see below) — resolved by adding `DomainPack.customLogin` as a generic, mutually-exclusive alternative to `auth` (`src/types/domain-pack.ts`, `ConflictingAuthConfigError` guard in `runDiscovery`), used by `packs/horse-haven-ops/domain-pack.ts` via `@clerk/testing/playwright`. Added two new goals (`view-dashboard`, `check-feed-board`) plus `browse-animals` (not yet exercised — small sample didn't draw it), two new personas (reused from `examples/archetypes.ts`), and a new Clerk-backed test-volunteer seed script in `volunteer-ops`. Validated with a real small-scale run (`sim.config.smoke.ts`, new — reusable for future mechanical checks): 4/4 sessions completed, zero findings, $0.0233. One real integration bug found and fixed along the way (`@clerk/testing`'s `clerk.signIn` reads `process.env.CLERK_SECRET_KEY` directly, ignoring `clerkSetup`'s explicit option — see `GAPS.md`'s 2026-07-29 customLogin entry). Full detail in that same `GAPS.md` entry and `CLAUDE.md`'s Orchestrator section.
