...
**Status: done, 2026-09-10 — not yet committed, review the diff.** Grader Sessions 6–8 landed first (confirmed via `FUTUREPLAN.md`'s own status entries and `git log` — Layers 4–7, `runGrading`, the Grading report, the CI JSON contract, and a real reference-pack validation run all exist now), unblocking this session per ADR 0008. `src/soak/grader-adapter.ts`'s `turnToCase(turn, rubricKeyFor, context?)` produces a Grader `CaseInput` — not a persisted `Case`: a `Case` additionally carries `id`/`gradingRunId`/`createdAt`, which only exist once a real Grading Run has actually inserted it, and a soak turn has no Grading Run at adapter-construction time. `CaseInput` (`{input, output, rubric}`) is the actual shape `GraderPack.loadCases()` returns and the only one genuinely available here — CTS.md's own "Case" wording is the glossary term, not literally the persisted TS type, confirmed by checking Grader's real `types.ts` before writing this. `rubricKeyFor` is the caller-supplied seam (ADR 0010) that keeps target-specific rubric-selection knowledge out of Drover's repo entirely — this file has no Shenny-specific (or any target-specific) code anywhere.

One shape decision beyond CTS.md's literal two-line sketch: `CaseInput.input` is always a consistent `{ requestPayload, context? }` envelope (`TurnCaseInput`, exported), never sometimes-bare-`requestPayload`-sometimes-not depending on whether a caller passes context. CTS.md's own Session 5 item 2 named the real need directly — "`input` carries the turn's `requestPayload` plus whatever context a groundedness check needs (e.g. the source records an output is supposed to be grounded in)" — which requires a place for a caller to actually attach that context; `turnToCase` takes it as an optional third parameter, threaded into the envelope only when supplied (checked against Layer 5's real implementation, which treats `input` fully opaquely — just JSON-stringified into the judge prompt — so any consistent shape here is equally valid to it).

Verified against Grader's real types and real static validator, not a duplicated shadow schema: one test builds a minimal real `GraderPack` whose `loadCases()` returns a `turnToCase(...)`-produced `CaseInput`, and feeds it through Grader's actual `validateGraderPack` (`src/grader/pack-validation.ts`) — asserting it resolves cleanly with the exact same `CaseInput` object round-tripping through, and a companion test confirming a bad `rubricKeyFor` result still fails Grader's own real `unknown-rubric` check. No `runGrading`/real judge dispatch was needed or run — Session 5's own cost basis is genuinely `$0`, and `validateGraderPack` alone (no LLM calls) is the correct-weight verification for a pure data-shape adapter. 6 new tests in `tests/soak/grader-adapter.test.ts`. Full project suite (449 tests, up from Grader Session 8's 443), `tsc`/`tsc --noEmit`/`biome check`/`npm run build` all clean.

Was **BLOCKED until Grader Sessions 6–8 land** (Layer 5 groundedness, the full layer tie-together via `runGrading`, and Grader's report/CLI didn't exist yet at planning time — see ADR 0008); confirmed unblocked and completed above.
...
**Status: done, 2026-09-10 — not yet committed, review the diff.** `src/soak/digest.ts` adds `TurnDigest`/`buildTurnDigest` — architecturally mirrors `buildSessionDigest`'s role but needs no `db` parameter at all, since a `TurnRecord` already carries everything a digest needs directly (request/response payload, timing, error state), unlike sessions/events/findings which need a join. `DEFAULT_TURNS_PER_CHUNK = 50` is the flagged guess CTS.md's own "explicitly still open" note asked for — larger than `DEFAULT_SESSIONS_PER_CHUNK`'s 25 since a multi-hour soak run's turn count can run far higher than a discovery run's session count. `src/soak/cross-turn-prompt.ts` builds the system/user prompt (three LLM-derived categories: `disagreement`, `drift`, `recurring-error-cluster` — explicitly told *not* to report timing, which is handled separately). `src/soak/cross-turn-provider.ts` mirrors `BatchAnalystProvider`'s exact create/poll/fetch-results Batch API lifecycle (`CrossTurnProvider`/`BatchCrossTurnProvider`/`ScriptedCrossTurnProvider`) against a genuinely different tool schema (`report_cross_turn_findings`, `turnIds` not `sessionIds`, no `route` field) — not a literal reuse, per ADR 0008, though `computeCostUsd`/`TokenUsage`/`BATCH_DISCOUNT` *are* reused directly from the actor/analyst tiers since those are genuinely provider-shape-agnostic. `src/soak/cross-turn-validate.ts` mirrors the Analyst tier's `validate.ts` discipline exactly (malformed findings logged and skipped, never crash the pass; turn ids not copied from the given digests are filtered out). `src/soak/cross-turn.ts`'s `runCrossTurnAnalysis` ties it together: chunk → one `provider.analyze()` per chunk via `Promise.all` → validate → merge with `detectTimingAnomalies`'s deterministic findings.

Timing-anomaly detection reuses `src/stampede/metrics.ts`'s `percentile()` function directly (the literal math, per CTS.md's own wording), grouping turns by lane itself (grouping logic is soak-specific, not "new statistics code"). A lane is flagged when `p99 >= p50 * TIMING_ANOMALY_P99_TO_P50_RATIO` (default `3`, flagged as a guess) with at least `MIN_SAMPLES_FOR_TIMING_ANOMALY` (default `5`, also flagged) timed turns — both constants exported so a future session can tune them once real run data exists, same "pick something reasonable now, revisit with real data" precedent every other guessed constant in this project follows. The finding's `turnIds` are the specific outlier turns at or above that lane's own p99 (not every turn in the lane) — a more useful pointer for whoever reads the finding. This half of the pass is fully deterministic and runs across *every* turn regardless of chunk boundaries, so — unlike the LLM-derived findings — it has no cross-chunk correlation gap.

One scope boundary worth being explicit about: Session 6 produces `CrossTurnFinding[]` **in memory only** — no new `soak.sqlite` table, no persistence, no match-key. CTS.md's own Session 7 description ("persists both sets of findings") makes clear that's deliberately Session 7's job, not this one's; Session 6's stop condition only asks for the pass to run and surface a planted pattern, which `runCrossTurnAnalysis` does.

Verified via the CTS.md-specified planted-pattern scenario: two turns referencing the same synthetic `recordId` with contradictory `toneEval` outputs, detected via a `ScriptedCrossTurnProvider` configured to report the `disagreement` finding a real judge model would have found — proving the full select-digest→chunk→validate→aggregate pipeline end-to-end without any real LLM cost, exactly matching this session's own `$0` cost basis. 29 new tests across `tests/soak/{digest,cross-turn-prompt,cross-turn-provider,cross-turn}.test.ts` (digest mapping/truncation, prompt content, the real Batch API lifecycle via the same `vi.mock("@anthropic-ai/sdk", ...)` precedent `tests/analyst/provider.test.ts` established, the planted-pattern stop condition itself, malformed-finding skipping, multi-chunk aggregation, and `detectTimingAnomalies`'s threshold/sample-size/per-lane-independence behavior). Full project suite (478 tests), `tsc`/`tsc --noEmit`/`biome check`/`npm run build` all clean.
