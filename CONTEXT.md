...
**Soak mode** — added 2026-09-09, planning complete via a `/grill-with-docs` session (see `CTS.md`, `docs/adr/0006`–`0010`), not yet built. A separate subsystem (not a `DomainPack`/tier extension — see Glossary), driving a target over direct HTTP calls rather than the browser, continuously for hours at a time via a local Ollama driver model, to surface the failures that only show up at volume — the edge case on the 4,000th weird-but-plausible interaction, or a "silent" error that never throws (bad AI-pipeline output, a guardrail quietly leaking, drift between related outputs). First target: Shenny.
...
A separate subsystem (not a fourth tier, not a `DomainPack` extension — see `docs/adr/0006`–`0010`), added 2026-09-09, planning complete, not yet built. Full session-by-session build plan lives in `CTS.md`.

- **Soak mode** — Drover's continuous long-running mode: a local Ollama driver model grinds through a `Blueprint` against a target over direct API calls (not the browser — see `docs/adr/0006`) for hours at a time, logging every turn to its own SQLite file. Own CLI entry point (`drover soak run|analyze|report`); does not participate in the actor/analyst/fixer `runs`/`sessions`/`action_events` lineage, same non-participation Grader already established for itself. Named for the industry term "soak testing" — sustained, realistically-paced usage over a long duration, distinct from **Stampede**'s rapid-fire load-testing volume.
  _Avoid_: Continuous mode (CTS.md's original working name — superseded once "Soak" was adopted as the precise, already-established industry term)
- **Blueprint** — the versioned, typed-TypeScript config a soak run is built from (`SoakBlueprint`, mirrors `DomainPack`/`GraderPack`'s "typed config, default export, loaded via `loadDefaultExport`" precedent — see `docs/adr/0006`'s sibling decision that soak content is TS, never JSON, consistent with every other config surface in this project). Carries `targetBaseUrl`, `dataPolicy`, a `variationPool` (a Claude-authored bank of example narratives per variation bucket — see `docs/adr/0009`), per-pipeline call budgets, pacing range, a hard-dollar `SoakBudget` ceiling, `maxDurationHours`, and an optional `teardown` hook (same timestamp-window-sweep contract `DomainPack.teardown` already established, since the non-negotiable "staging teardown wipes everything a run created" constraint applies here too). App-specific blueprints (Shenny's real one) live in the target's own repo when it has one, not in Drover's — see `docs/adr/0010`.
- **Turn** — one iteration of the soak loop: pick the next variation (plain scheduling code, no LLM call — see `docs/adr/0009`'s consequence), lightly vary its example text via the local driver model, dispatch it over HTTP, log the result (`requestPayload`/`responsePayload`/`httpStatus`/`responseTimeMs`/`explicitError`). The soak-mode analogue of Drover's `ActionEvent`, but HTTP-shaped rather than browser-action-shaped.
  _Avoid_: Action (collides with `ActionEvent`'s browser-primitive meaning), Request (too generic — a Turn is a full logged unit, not just the outbound call)
- **Backbone traffic** — the high-frequency lane of a soak run: turns against a target's uncapped/unmetered surface (for Shenny, journal Entry creation, which triggers ToneEval/EntrySplit uncapped at every tier), paced by the blueprint's `pacingMs` range. Distinguished from **metered-pipeline traffic** — the target's rate-limited/usage-capped surface (for Shenny, MessageAnalysis/RecordingReport/InsightReport/Clarity) — which is driven by its own explicit per-run call budget instead of `pacingMs`, since uniform pacing across both would exhaust real monthly caps within the first hour of an unattended run.
- **SoakBudget** — the hard-dollar ceiling on a soak run, checked between turns with graceful shutdown (never dies mid-write) — same load-bearing-budget-knob discipline `SessionBudget`/`analystCeilingUsd`/`graderCeilingUsd` already established elsewhere in this project, extended here because Soak mode is the first mode that can otherwise run real Sonnet-calling backbone traffic unattended for hours with no cost ceiling at all.
- **Turn digest / cross-turn pass** — the soak-mode analogue of the Analyst tier's session digest and cross-session pattern mining, architecturally mirrored (chunked, Sonnet Batch API, map-reduce) rather than code-shared, since a Turn isn't a session — see `docs/adr/0008`. Finds what a single-turn judge structurally can't: two pipelines disagreeing about the same underlying record, drift across near-identical turns, response-time anomalies (reusing `src/stampede/metrics.ts`'s percentile math directly). Complements, rather than replaces, feeding individual turns to **Grader** as `Case`s for single-turn content judging (guardrail leaks, groundedness) — see `docs/adr/0008`.
