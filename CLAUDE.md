...
**Next step:** Session 9 — the real Horse Haven Ops domain pack and a validation run against Horse Haven staging. Its one concrete technical blocker (nothing wired `treeline/adapter.ts`'s `performLogin` into `SimConfig`/`DomainPack`/`runDiscovery`, so no persona could reach anything behind an auth wall) is now fixed — see the Orchestrator section's `DomainPack.auth` entry below. What's left is external, not engineering: no `ANTHROPIC_API_KEY` and no confirmed `volunteer.horsehaventn` staging URL/credentials have been available in any build environment so far, so the real pack build-out and validation run are still blocked on those two inputs, not on anything Drover itself is missing. Treat CONTEXT.md's relevant sections ("Open source packaging") as the spec for that work when it's picked up. Note: a real-app validation exercise has already happened ad hoc against a local `volunteer-ops` checkout (see `KIOSK.md`, `packs/horse-haven-ops/`) — read that before continuing Session 9 rather than duplicating it; it's scoped to the kiosk check-in flow only (the one route with no auth wall), not the full Horse Haven Ops domain pack CONTEXT.md describes.

**Not yet built:** the full real Horse Haven Ops domain pack (every flow, not just kiosk check-in, now that `DomainPack.auth` makes them technically reachable) and a validation run against actual Horse Haven staging (`volunteer.horsehaventn`, not the local `volunteer-ops` checkout `KIOSK.md` used). This maps to the original plan's Session 9 scope.
