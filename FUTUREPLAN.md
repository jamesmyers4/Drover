...
**Status: done, 2026-07-31 — not yet committed, review the diff.** `src/grader/consensus.ts`'s `runConsensusRound` dispatches every judge sequentially (ADR 0004), computes per-Check agreement via each Check's own rule (`checksAgree`: exact-match for boolean, `numericTolerance`-bounded spread for numeric — never a Round-wide setting, Q11), and for any Check that disagrees, dispatches one scoped escalation Task via a synthesized single-Check `Rubric` (`singleCheckRubric`) plus a framing string (`buildEscalationFraming`) that embeds every disputing judge's model family, value, and reasoning verbatim — the shared Case input/output rides along unchanged through the normal `GraderScoreRequest` shape. An agreed Check's `finalValue` is the common value for boolean, the mean for numeric; an escalated Check's `finalValue` is the escalation Task's own adjudicated value. `RunConsensusRoundResult` deliberately has no aggregate pass/fail field — the Consensus Round glossary entry is explicit that a round's result is "never collapsed into one opaque pass/fail," so the result surfaces `checkResolutions` (one per Check, in rubric order) plus the persisted judge/escalation `Task[]` arrays and lets a caller derive whatever summary it needs.

`src/grader/scheduler.ts` gained the two guardrails FUTUREPLAN.md named for it: `assertDistinctModelFamilies` (throws `InsufficientJudgesError` below 2 judges, `DuplicateModelFamilyError` on a shared `modelFamily` — ADR 0003), called once at the start of `runConsensusRound`; and `assertEscalationDispatchAllowed` (a thin wrapper re-calling `provider.ts`'s `assertHostedGraderDispatchAllowed`), called immediately before every escalation dispatch as the scheduler-level defense-in-depth half of ADR 0002's "one chokepoint isn't trusted alone." `src/grader/budget.ts` is new: `GraderBudget` tracks a running `spentUsd` and `assertCanDispatch()` throws `GraderBudgetExceededError` once spend has already reached `ceilingUsd` — checked immediately before each escalation call (mirroring the `dataPolicy` gate's own per-dispatch timing, both right before the same call site in `consensus.ts`), not a pre-flight estimate; a call already in flight when the ceiling is crossed is still allowed to finish, since real cost isn't knowable before a call returns. `src/grader/layers/judge-layer.ts`'s internal `checkPasses` is now exported so `consensus.ts` reuses the exact same single-judge verdict rule for a judge Task's own status and an escalation Task's adjudicated verdict, rather than a second copy of the same logic.

One scope decision worth flagging explicitly, deliberate: **`runConsensusRound` isn't wired into `DEFAULT_LAYER_REGISTRY`/`runGradingRun` at all.** No real multi-judge layer exists yet to consume it — Layers 4-7 are Session 6's job — so this mirrors exactly how Session 4 proved Layer 2/3's dispatch mechanics via an explicit registry/direct calls before Session 3's scheduler ever wired them in. `consensus.ts` is tested directly (`tests/grader/consensus.test.ts`), not through `runGradingRun`.

**Post-review fix: a judge/escalation provider throwing needed to be treated as infrastructure noise, not silently identical to a guard refusing to proceed.** The first pass let every error inside `runConsensusRound` propagate uncaught, with no distinction between "a judge's provider call failed" and "a policy/budget guard refused to proceed" — review caught that these are categorically different (a provider throw says nothing about the graded content; a guard failing is a deliberate, non-retriable stop) and that conflating them would make Q10's escalation-rate metric uninterpretable (a creeping rate from provider flakiness looks identical to one from genuine judge disagreement). Fixed in three parts:
- Judge and escalation dispatch calls now retry through `dispatchWithRetry` (`maxDispatchAttempts`, default 3 — "one or two retries" — with linear backoff, an injectable `sleep` so tests don't actually wait) before being treated as a real failure. Guard calls (`assertDistinctModelFamilies`, `assertEscalationDispatchAllowed`, `GraderBudget.assertCanDispatch`) are never wrapped in this retry — they still throw immediately and uncaught, exactly as before.
- `ConsensusRoundStatus` (`types.ts`) gained a third state, `aborted-error`, alongside `pending`/`resolved` — mirroring the precedent Q5 set for `TaskStatus`'s `skipped`: an aborted round is neither "not yet resolved" nor "resolved with a gap," and collapsing it into either would corrupt what that category means to a report. `ConsensusRound` gained `abortedReason`/`abortedAt`; migration 2 (`consensus-round-abort-state`) adds `status`/`aborted_reason`/`aborted_at` columns, `status` defaulting to `'pending'`. `GraderDb.resolveConsensusRound` now also sets `status = 'resolved'`; a new `GraderDb.abortConsensusRound` sets `status = 'aborted-error'` plus the reason/timestamp, and persists whatever Checks were actually resolved before the failing dispatch (may be a strict subset of the rubric, never discarded).
- Once every retry is exhausted for a judge or escalation call, `runConsensusRound` no longer throws — it calls the new `abortConsensusRound` and **returns** `{ status: "aborted-error", abortedReason, checkResolutions, judgeTasks, escalationTasks }` instead, since this is a first-class, expected-to-happen-sometimes outcome the caller shouldn't need exception handling to discover. Already-persisted judge/escalation Task rows from earlier in the same round are never rolled back — confirmed directly by a test asserting a surviving judge's Task row is still queryable after the other judge's dispatch exhausts its retries and the round aborts.

**No real escalation call was made.** Every test uses `ScriptedGraderProvider` (including a small `FamilyScriptedProvider` test subclass that overrides `modelFamily`, since the real class always reports `"scripted"`, plus `AlwaysFailingProvider`/`FlakyThenSucceedsProvider` for the retry/abort paths) — there's no real `GraderPack`/Ollama install yet to run a genuine end-to-end Consensus Round against (same unverified-against-real-infra caveat Session 4 already flagged for Ollama), and per this session's own cost-basis note, real escalation cost was never going to be incurred until that exists. 27 new tests (`tests/grader/consensus.test.ts`: 15, `tests/grader/budget.test.ts`: 5, `tests/grader/scheduler.test.ts` additions: 6, plus one new `abortConsensusRound` round-trip test in `tests/grader/db.test.ts`) bring the full suite to 358, all passing; `tsc`/`tsc --noEmit`/`biome check` all clean on every file this session touched (the same pre-existing, unrelated CRLF lint issue on 6 files this session never touched — flagged by Session 4 — remains on `main`).
...
**Status: done, 2026-09-10 — not yet committed, review the diff.** `src/grader/ci-summary.ts`'s `buildGraderCiSummary` reshapes an already-built `GradingReport` (Session 6) into the versioned, machine-facing contract (`schemaVersion: 1`) rather than re-querying `GraderDb` a second time — deliberately drops each Case's `input`/`output` (a content-review concern for the markdown report, not this artifact) and each Case's rubric criteria text (kept once, self-contained, in a top-level `rubricsUsed` map instead of repeated per Case), while preserving every Check's own name/value/reasoning in full — never collapsed to bare pass/fail, per ADR 0005. `drover grade` gained `-j, --json [path]` (opt-in — printed to stdout with no path, written to a file with one, omitted entirely skips it, since a human running this interactively has no use for a JSON blob cluttering their terminal the way the markdown report's own always-on default earns its place). The exit-code contract itself needed no new logic: a malformed pack or a genuine `crashed` run already propagated as a thrown error into the CLI's existing top-level catch (`process.exitCode = 1`) — Session 7's job here was making that deliberate and documented rather than incidental, including the explicit call that this is *not* a content-quality gate (every Check failing still exits 0, as long as the run itself completed structurally). `docs/GRADER-CI.md` is the new deployment-facing doc for whoever wires Shenny's CI: the self-hosted-runner requirement and *why* (judge-pool network reachability, not CLI behavior), the new network-exposure dimension ADR 0005 flagged, the full/scheduled-not-PR-gating run-scope decision, and an explicit, unmissable §5 stating the fail-threshold policy is genuinely unbuilt — a CI job must parse the JSON summary itself and decide what "failed" means, not read `drover grade`'s exit code as a proxy for content quality. 5 new tests in `tests/grader/ci-summary.test.ts` (including a golden-file snapshot of the JSON shape itself, guarding the durable contract against silent drift) plus a real, non-scripted CLI smoke run confirming both directions of the exit-code contract (0 on a real completed run against real Ollama+Anthropic judges, 1 on a pack referencing an unknown rubric) and a real `--json` file write. Full project suite (443 tests), `tsc`/`biome` both clean. Full detail in the session transcript.

**Cost: $0 for this session's own new code — it's output/documentation work.** The manual CLI verification run did spend a handful of real, tiny Ollama+Anthropic calls (see the session's own default-routing note in `src/cli/index.ts`) — negligible, not new logic.
...
**Status: done, 2026-09-10 — not yet committed, review the diff.** `examples/grader-toy-pack/pack.ts` is 5 Cases across 2 rubrics (`tone-eval` boolean, `accuracy-eval` numeric) — a clean pass and clean fail on each rubric, plus one Case (a factually-accurate core claim with an unsupported trailing embellishment) whose escalation potential was empirically probed against the real judge pair before being written into the pack, not guessed at. Ran for real via `drover grade` against real local Ollama (`qwen3:8b`) and real Anthropic (`claude-haiku-4-5`, wired in as the CLI's own default second judge as of Session 6/the "wire in anthropic key" follow-up) — no `ScriptedGraderProvider` anywhere in this run. All 7 layers dispatched (`layers 4-7 enabled: 4, 5, 6, 7`), the run completed (22 Tasks passed, 13 failed, 0 skipped), and the ambiguous Case genuinely escalated **twice** — Layer 5 and Layer 6 each ran their own independent Consensus Round on the same Case/output and landed on different verdicts (Layer 5's escalation adjudicated below the pass threshold, Layer 6's above it), a real demonstration of per-layer framing actually changing what the judges attend to, not a scripted illustration of it. `examples/grader-toy-pack/REFERENCE-RUN.md` is the plain-language summary this session's own stop condition asks for — full case-by-case breakdown, the escalation explained in detail, and an honest caveat (found while building this) that this Case's escalation isn't perfectly deterministic run-to-run against real models, so a pack author leaning on it for guaranteed CI escalation coverage should track the aggregate rate, not assert a single Case always escalates. `tsc`/`biome`/full suite (443 tests) all clean — this session added no new source code beyond the example pack + its summary doc, so no new automated tests were needed. Full detail in the session transcript.

**Cost: real, small.** A handful of `claude-haiku-4-5` calls across the reference run (4 consensus layers × 5 Cases, one judge call each plus 2 real escalation calls) — cents, not dollars. Ollama's own calls are $0 as always. Not tracked/summed by the tool itself (GAPS.md's cost-tracking entry still applies) — estimated here from the call count.
